...
vector<double> serial_pr(map<int, vector<int> >& file_nodes, double alp, int num_threads, int total_nodes, vector<M*> locks){
  vector<double> pr_new(total_nodes, 0);
  vector<double> pr_old(total_nodes, 1.0/sqrt(total_nodes));
  M mri, md, msum, merror;
...
int main(int argc, char* argv[]){
  ...
  read_data_to_adjancy_list("com-youtube.ungraph.txt", file_nodes, total_nodes);
  ...
  for(alp = 0.6; alp < 1; alp += 0.05){
	...
	cout << "alpha: " << alp << ", num_threads: " << num_threads << endl;
  	...
  	vector<double> pr = serial_pr(file_nodes, alp, num_threads, total_nodes, locks);
